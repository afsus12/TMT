...
class UserServices {
  Future<http.Response> login(userLogin data) {
    return http.post(
  ...
  Future<http.Response> registration(userRegistration data) {
    return http.post(
  ...
  Future<int> connected() async {
    ...
    print(response.body);
    print(userEmail);
    if (response.statusCode == 200 && validAccount == true && userEmail != "") {
      if (org != null && org != "") {
        return 1;
      } else {
        return 2;
